...
export function TransactionDialog({ open, onOpenChange, transaction = null }: { open: boolean; onOpenChange: (open: boolean) => void; transaction?: FinanceTransaction | null }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[calc(100dvh-1.5rem)] w-[calc(100%-1.5rem)] overflow-y-auto border-glass-border bg-background/95 p-4 text-foreground backdrop-blur-2xl sm:max-w-2xl sm:p-6">
    <DialogHeader><DialogTitle className="font-display text-xl sm:text-2xl">{transaction ? "Editar lançamento" : "Novo lançamento"}</DialogTitle><DialogDescription>{transaction ? "Atualize somente este lançamento da série." : "Registre uma receita ou despesa. Parcelas futuras são distribuídas automaticamente."}</DialogDescription></DialogHeader>
    <form onSubmit={submit} className="mt-1 space-y-4 sm:mt-2 sm:space-y-5">
      ...
      <div><Label>Forma de pagamento</Label><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">{[["credit","Cartão"],["debit","Débito"],["pix","Pix"],["cash","Dinheiro"]].map(([value = "pix",label]) => <Button className="min-h-11 text-xs sm:min-h-9 sm:text-sm" key={value} type="button" variant={method === value ? "default" : "glass"} onClick={() => setMethod(value)}>{label}</Button>)}</div></div>
